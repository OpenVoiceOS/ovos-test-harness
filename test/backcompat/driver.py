...
def entity_blacklist_emission_supported(skill_python: str) -> bool:
    ...
    call #454 added (``ovos_workshop/skills/ovos.py``); its ABSENCE from
    the installed registration source means this vintage predates
    blacklist emission entirely. The call has since moved out of
    ``register_entity_file`` into the per-language helper
    ``_register_entity_file_for_lang`` (ovos-workshop 9.8.4a1 reads it
    there), so the probe reads both and a vintage with no helper still
    answers on ``register_entity_file`` alone. Reading only the outer
    method made this probe answer False on a vintage that does emit,
    which turned cell (d)'s marker on and produced an XPASS(strict) -- verified live that #454 first shipped in
    ...
    script = (
        ...
        "helper = getattr(OVOSSkill, '_register_entity_file_for_lang', None)\n"
...
def set_context_writes_session_intent_context(skill_python: str) -> bool:
    """Whether THIS S-axis venv's ``OVOSSkill.set_context`` writes the
    CONTEXT-1 §5.0 session surface (``Session.set_intent_context`` on the
    session bound to the dispatch message) -- ovos-workshop#527, merged
    2026-08-13, the skill half of the #527/#857 pair
    ``test_backcompat_session.py`` is gated on.

    Real-symbol probe via source inspection, like
    :func:`entity_dual_emit_supported`: pre-#527 ``set_context`` only
    emits the legacy ``add_context`` frame, so the frame carries no
    ``intent_context`` write and the reachability cell is a real gap
    there; post-#527 the write rides the forwarded frame (CONTEXT-1
    §5.3) and the cell is a plain pass. A version compare would answer
    for the pin, not for the installed surface.
    """
    script = (
        "import inspect\n"
        "import ovos_workshop.intents as intents\n"
        # #527 lands the write in the adapt-side helper class the public
        # facade delegates to; a vintage without that class keeps the
        # write (if any) on IntentServiceInterface.set_context itself.
        "owner = getattr(intents, '_AdaptIntentApi', intents.IntentServiceInterface)\n"
        "method = getattr(owner, 'set_context', None)\n"
        "print(method is not None and 'set_intent_context' in inspect.getsource(method))\n"
    )
    return _skill_venv_probe(skill_python, script) == "True"
...
def core_dealiases_entity_names() -> bool:
    """Whether the core-side padatious pipeline plugin folds the munged
    entity name at registration -- ``_dealias_entity_name``, the symbol
    ovos-padatious-pipeline-plugin#95 adds.

    The entity-side counterpart of :func:`core_canonicalizes`, and the real
    symbol rather than a version string for the same reason: #95 merged on
    2026-08-13 and ships in ovos-padatious 2.2.0a1, so the entity cells it
    unblocks must read the installed surface, not a pin.
    """
    try:
        import ovos_padatious.opm as opm
    except ImportError:
        # A venv whose matcher does not import (the
        # venv_core_old_matchers_new pin skew, ovos-padatious 2.2.0a1
        # against ovos-spec-tools 1.10.0a1) must not abort collection
        # for the whole cell here; the cells that need the plugin fail
        # at their own fixture with the real ImportError instead.
        return False
    return hasattr(opm, "_dealias_entity_name")
...
def make_padatious_pipeline(bus: MessageBusClient):
    ...
    ``make_intent_service``/``make_stop_service``. Used by entity cell
...
def make_stop_service(bus: MessageBusClient):
    ...
    ``make_intent_service``. Used by the T2.4 stop scenario's #802
