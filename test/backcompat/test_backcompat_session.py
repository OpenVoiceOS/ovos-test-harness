...
That reachability was BROKEN before a real, cited fix pair:
``ovos-workshop#527`` (fix/set-context-original-key) + ``ovos-core#857``
(fix/mirror-context-resolved-key), both merged on 2026-08-13 (CAMPAIGN.md
W1B-L1). #527 ships in ovos-workshop 9.8.x and is the half this file can
see: the skill venv is the S axis, and the reachability cell's marker
reads that venv's installed ``set_context`` source
(``driver.set_context_writes_session_intent_context``), not a pin. On a
pre-#527 skill vintage the chain, read directly off
``ovos_workshop/skills/ovos.py`` and ``ovos_workshop/intents.py``, is:
...
on a skill vintage whose ``set_context`` has no session write, naming both
PR numbers. The positive control -- a DIRECT ``intent_context``
...
from .driver import (CONTEXT_SET_DONE_TOPIC, CONTEXT_SET_TRIGGER_TOPIC,
                     ...
                     session_context,
                     set_context_writes_session_intent_context)
...
#: ovos-workshop#527 merged on 2026-08-13. Read the installed skill-side
#: surface, not a pin: on a vintage whose ``set_context`` writes
#: ``Session.intent_context`` the reachability cell is a plain pass, and
#: an unconditional marker turns it into an XPASS(strict) failure.
_SKILL_WRITES_SESSION_CONTEXT = (
    set_context_writes_session_intent_context(SKILL_PYTHON)
...
_CONTEXT_XFAIL_REASON = boundary_xfail(
    boundary="OVOSSkill.set_context (alphanumeric_skill_id + context write "
             "dialect) writes no Session.intent_context entry on this "
             "skill vintage, so the forwarded add_context frame carries "
             "nothing a requires_context gate could read",
    axes=("S", "C"),
    blocked_on="ovos-workshop#527, ovos-core#857",
    owner="ovos-core",
    note="both merged 2026-08-13; the marker is probe-derived "
         "(driver.set_context_writes_session_intent_context) and only "
         "applies to a pre-#527 skill vintage -- CAMPAIGN.md W1B-L1. "
         "XPASS here means the probe read False on a vintage that does "
         "write the session, a probe bug, not a product signal.")
...
@pytest.mark.xfail(not _SKILL_WRITES_SESSION_CONTEXT, strict=True,
                   reason=_CONTEXT_XFAIL_REASON)
@pytest.mark.axes("S", "C")
def test_a_context_set_via_set_context_is_reachable_by_the_intent_service(stack):
    ...
    try:
        ...
        # core instance in this fixture). Post-#527 the skill writes the
        # entry on the session bound to its dispatch message and the
        # add_context frame is ``forward``-derived from that message, so
        # the write rides the frame (OVOS-CONTEXT-1 §5.3: "Any Message the
        # handler separately derives via MSG-1 `forward` from the dispatch
        # Message carries the write to the orchestrator"). Read it off the
        # captured frame. NOT off ``SessionManager.sessions``: OVOS-SESSION-2
        # §2.2 -- "Between utterances on a named session, the orchestrator
        # holds no state for that session" -- and a bus-client that honours
        # it (2.11.x, the new-core driver) keeps no registry entry for a
        # named session, so a registry read answered None there while it
        # passed by accident on the pre-§2.2 old-core client, which still
        # folded every observed frame into its registry.
        from ovos_bus_client.session import Session
        session = Session.from_message(add_ctx.messages[0])
        assert session.session_id == session_id, (
            f"{COMBO}: add_context frame carries session "
            f"{session.session_id!r}, expected {session_id!r}")
