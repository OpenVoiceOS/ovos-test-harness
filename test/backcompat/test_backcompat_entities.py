...
#: ovos-padatious-pipeline#95 merged on 2026-08-13 and ships in
#: ovos-padatious 2.2.0a1. Cells (a) and (c) are blocked on it, so
#: their markers must read the installed CORE-side matcher rather
#: than stay unconditional: on a matcher that carries the fold they
#: are plain passes, and an unconditional marker turns each of them
#: into an XPASS(strict) failure instead.
_CORE_DEALIASES_ENTITIES = core_dealiases_entity_names()
...
@pytest.mark.xfail(not _CORE_DEALIASES_ENTITIES, strict=True,
                   reason=_SLOT_BIND_XFAIL_REASON)
@pytest.mark.axes("S", "C", "M")
...
@pytest.mark.xfail(_HAS_DUAL_EMIT and not _CORE_DEALIASES_ENTITIES,
                   strict=True, reason=_EXACTLY_ONCE_XFAIL_REASON)
@pytest.mark.axes("S", "C", "M")
